...
def parse_args():
    ...
    parser.add_argument("--content_dir", type=str, default="A:/NST_code/content_data",
                    help = "Location of the content images")
    parser.add_argument("--style_dir", type=str, default="A:/NST_code/style_data",
                    help = "Location of the style images")
    parser.add_argument("--vgg", type = str, default = "A:/NST_code/vgg_normalised.pth",
                    help = "Location of the vgg model")
    ...
    parser.add_argument("--crop", action = "store_true",default=True,
                    help = "Whether to crop the images to the final size")
...
def main():
    ...
    content_transformer = get_transformer(args.content_size, args.crop,args.final_size)
    style_transformer = get_transformer(args.style_size, args.crop,args.final_size)


    content_dataset = ImageDataset(args.content_dir, transform = content_transformer)
    style_dataset = ImageDataset(args.style_dir, transform = style_transformer)
    ...
    style_dataloader = DataLoader(style_dataset,
                                  ...
                                  pin_memory=True,
                                  drop_last=True)
    print(f"Content dataset size: {len(content_dataloader)}")
    print(f"Style dataset size: {len(style_dataloader)}")  
